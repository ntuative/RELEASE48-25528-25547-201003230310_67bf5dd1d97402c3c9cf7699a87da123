package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_422 >= var_310.length)
         {
            return "";
         }
         var petColor:IPetColor = var_310[var_422] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_603[var_1558] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(var_422 < 0 || var_422 > var_310.length)
         {
            var_422 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_589.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_137,onTextWindowEvent);
         var_589 = new Map();
         ...
         var_883 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_310 = [];
         var_603 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_883);
               if(_loc11_)
               {
                  var_603 = _loc11_.breeds;
                  var_310 = _loc11_.colors;
                  if(var_310 == null)
                  {
                     var_310 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_603.length)
         {
            _loc12_ = var_603[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_883,_loc12_.id),getRaceLocalizationKey(var_883,_loc12_.id)));
            _loc6_++;
         }
         var_589.add(var_883,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_976,onApproveNameResult);
         _loc5_ = var_589.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_310)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_589.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_422 >= 0 && var_422 < var_310.length)
         {
            _loc7_ = var_310[var_422] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_603[var_1558] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_883,_loc3_.id,_loc2_,AvatarScaleType.const_51);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_310,AvatarAction.const_614);
         _loc4_.endActionAppends();
         _loc4_.setDirection(AvatarSetType.const_37,2);
         setPreviewImage(_loc4_.getCroppedImage(AvatarSetType.const_37).clone(),true);
         _loc4_.dispose();
