package com.sulake.habbo.notifications
{
   ...
   public class HabboNotificationItemView implements IUpdateReceiver
   {
      ...
      private static const const_369:int = 1;
      ...
      public function HabboNotificationItemView(param1:IAsset, param2:IHabboWindowManager, param3:Map, param4:Map, param5:IHabboToolbar)
      {
         ...
         if(_loc7_ != null)
         {
            var_1344 = 0 - _loc7_.rectangle.bottom;
         }
         else
         {
            var_1344 = 15;
         }
         var_1572 = _window.height;
         var_616 = 4;
         var_438 = 0;
         var_32 = const_350;
         var_183 = param5;
      ...
      private function setNotificationText(param1:String) : void
      {
         ...
         _window.height = 0;
         _loc2_.text = param1;
         _loc2_.height = _loc2_.textHeight + var_1344;
         if(_window.height < var_1572)
         {
            _window.height = var_1572;
         }
      }
      ...
      public function update(param1:uint) : void
      {
         ...
         switch(var_32)
         {
            ...
            case const_369:
               var_905 += param1;
               _loc2_ = Number(var_905) / Number(var_437["time_fade_in"]);
               if(var_905 > int(var_437["time_fade_in"]))
               {
                  startDisplay();
               }
               adjustBlend(_loc2_);
            ...
            case const_787:
               var_1074 += param1;
               if(var_1074 > int(var_437["time_display"]) && !var_1345)
               {
                  startFadeOut();
               }
               break;
            case const_788:
               var_906 += param1;
               _loc3_ = 1 - Number(var_906) / Number(var_437["time_fade_out"]);
               adjustBlend(_loc3_);
               if(var_906 > int(var_437["time_fade_in"]))
               {
                  startIdling();
               }
         }
      ...
      public function reposition(param1:int = -1) : void
      {
         ...
         var _loc3_:int = var_183.orientation == "TOP" ? int(const_349) : 0;
         var _loc4_:int = var_183.orientation == "BOTTOM" ? int(_loc2_.height - const_349) : int(_loc2_.height);
         var _loc5_:int = var_183.orientation == "LEFT" ? int(const_349) : 0;
         var _loc6_:int = var_183.orientation == "RIGHT" ? int(_loc2_.width - const_349) : int(_loc2_.width);
         if(false)
         {
            if(false)
            {
               _window.x = _loc5_ + const_368;
            }
            else
            {
               _window.x = _loc6_ - 0 - const_368;
            }
         }
         else if(false)
         {
         ...
         if(false)
         {
            if(false)
            {
               _window.y = _loc3_ + var_616;
            }
            else
            {
               _window.y = _loc4_ - 0 - var_616;
            }
         }
         else if(false)
         {
