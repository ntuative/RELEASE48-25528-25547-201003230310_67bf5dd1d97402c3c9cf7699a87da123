package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1403:int = 5;
      ...
      protected function prepareLibrary() : void
      {
         ...
         if(var_358 == null)
         {
            failure("Failed to find resource class \"" + _name + "\" in library " + var_321.url + "!");
            return;
         }
         try
         {
         ...
         var_503 = new XML(bytes.readUTFBytes(bytes.length));
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         switch(param1.type)
         {
            case Event.INIT:
               ...
               var_446 |= LibraryLoader.STATE_EVENT_INIT;
               break;
            case Event.COMPLETE:
               debug("Load event COMPLETE for file \"" + var_15.contentLoaderInfo.url + "\"");
               var_446 |= LibraryLoader.STATE_EVENT_COMPLETE;
               break;
            case Event.ENTER_FRAME:
               break;
            case HTTPStatusEvent.HTTP_STATUS:
               var_368 = HTTPStatusEvent(param1).status;
               debug("Load event STATUS " + var_368 + " for file \"" + var_15.contentLoaderInfo.url + "\"");
               break;
            case Event.UNLOAD:
         ...
         if(var_446 == LibraryLoader.STATE_ANALYZE)
         {
            if(analyzeLibrary())
            {
               var_446 |= LibraryLoader.STATE_EVENT_FRAME;
            }
         }
         if(var_446 == LibraryLoader.STATE_READY)
         {
            var_1373 = true;
            prepareLibrary();
            removeEventListeners();
      ...
      protected function handleHttpStatus(param1:int) : Boolean
      {
         if(param1 >= 400)
         {
            if(var_932 > 0)
            {
               var_15.close();
               var_15.unload();
               addRequestCounterToUrlRequest(var_321,const_1403 - var_932);
               var_15.load(var_321,var_14);
               --var_932;
               return true;
            }
